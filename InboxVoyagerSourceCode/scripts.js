...
const CLIENT_ID = '967804298241-u1mvom6ldgek8t3rlsrniuh1sjntggma.apps.googleusercontent.com';//TODO
const API_KEY = 'AIzaSyAUPnOpJcYYSqrv1ci8DBPvmnmmMqF_UL4';//TODO
...
const SCOPES = 'https://www.googleapis.com/auth/gmail.readonly https://www.googleapis.com/auth/gmail.modify https://www.googleapis.com/auth/contacts.readonly https://www.googleapis.com/auth/userinfo.profile https://www.googleapis.com/auth/gmail.send https://www.googleapis.com/auth/userinfo.profile';
        ...
        if (messages.length > 0) {
            ...
            Promise.all(messagePromises).then(function () {
                ...
                allMessages.forEach(function (message) {
                    ...
                    prepareCenterdTimelineItem(message, sender);
                });
...
function prepareCenterdTimelineItem(responseResult) {
    ...
    // Extract the email address and fetch the sender's profile image
...
function getSenderProfileImage(email, timelineEntry) {
    ...
    if (senderImages[email]) {
        console.log("sender image: ",senderImages[email]);
        timelineEntry.find('.profile-picture').attr('src', senderImages[email]);
        return;
    }

    senderImages[email] = "./gmail-interface/svg/logo-3.svg";

    // If valid, proceed with fetching the profile image
    gapi.client.people.people.searchContacts({
        ...
        console.error('Error retrieving sender profile image:', error);
    });
}
